...
function EventDetails({
  ...
  return (
    <div className="detail-backdrop" role="presentation" onMouseDown={onClose}>
      <section ref={panelRef} className="detail-panel" role="dialog" aria-modal="true" aria-labelledby="event-detail-title" aria-describedby="event-detail-note" onMouseDown={(mouseEvent) => mouseEvent.stopPropagation()}>
        <button ref={closeButtonRef} className="icon-button" type="button" onClick={onClose} aria-label="Close event details"><Icon name="x" /></button>
        ...
        <p className="detail-note" id="event-detail-note">Dates can change. Confirm time-sensitive details with the official source.</p>
      </section>
    </div>
  );
}
...
export function CalendarExplorer() {
  ...
  return (
    <main>
                ...
                <section className="day-group" key={day}>
                  ...
                  <div className="day-events">
                    {events.map((event) => {
                      ...
                      return (
                        <button className="event-card" type="button" onClick={(clickEvent) => { setReturnFocus(clickEvent.currentTarget); setSelectedEvent(event); }} key={event.id}>
      ...
      {selectedEvent && <EventDetails event={selectedEvent} returnFocus={returnFocus} onClose={() => setSelectedEvent(null)} />}
    </main>
  );
}
